...
`vocab.select_intro_word` uses the same weighting over the introduction pool (`reps < INTRO_GRADUATION_REPS`, minus `remembered`) and ignores label filters; `scheduler.is_intro_slot` decides statelessly from `push_log` whether the next push draws from it.
...
- `scheduler.py`: `MIN_GAP_MIN = 45` (minimum spacing between consecutive pushes, enforced implicitly by the bucket algorithm), `INTRO_EVERY = 3` (every 3rd push of the local day is an introduction slot).
