...
| `/games` | Pick a vocab quiz from a 2-button menu. The Word → Translation game runs `min(10, vocab_size)` rounds: each round shows an English word and 4 inline buttons (1 correct translation + 3 distractors drawn from the same game's pool, randomly ordered); a tap edits the keyboard with ✅/❌ feedback and advances. After the last round the bot sends `🎯 You scored X/N`. Game state is in-memory only (a restart abandons in-flight games), and only one game per chat at a time — a second `/games` while a game is in progress replies `you have a game in progress`. With fewer than 4 vocab rows the menu is suppressed and the bot replies `add at least 4 words to your vocab first`. The Translation → Word slot is wired but currently replies `coming soon`. |
...
- **`games.py`** — pure scaffolding for `/games`: `Round`/`Game` dataclasses, `draw_rounds` (sample `min(10, N)` correct words + 3-distractor option sets per round, both without replacement; uses each row's stored `translation`), `apply_answer` (mutates score and current_round), `format_result`. No telegram imports; `bot.py` does the wiring and holds the per-chat in-memory map.
