...
| `/add <word or phrase>` | Add a word to this chat's vocab. Normalized to lowercased+stripped form. After a new word lands, the bot asks the LLM for a `pos:*` guess + up to 4 of this chat's existing labels that fit, and posts a follow-up message with one inline button per suggestion — tap to attach (button flips to `✅ <name>`), ignore to skip. LLM/empty-vocab failures are silent (no follow-up). |
| `/remove <word or phrase>` | Remove by exact (normalized) match. |
| `/list [<spec>...]` | List vocab words (least-mentioned first). With no args, shows every word. With one or more spec tokens (same `key:value` / bare-string syntax as `/label`), shows only words tagged with **all** listed labels (AND); each row also displays its labels. Empty result → `no words match those labels`. Malformed spec → `⚠️ malformed label spec: …` and no DB query. |
...
| `/games [<spec>...]` | Pick a vocab quiz from a 2-button menu: **Word → Translation** (prompt = English word, buttons = translations) or **Translation → Word** (prompt = translation, buttons = English words). With one or more spec tokens (same `key:value` / bare-string syntax as `/label`), the pool is restricted to words tagged with **all** listed labels (AND); the chosen filter survives the direction-picker tap. Either game runs `min(10, pool_size)` rounds with 4 inline buttons per round (1 correct + 3 distractors drawn from the same game's pool, randomly ordered); a tap edits the keyboard with ✅/❌ feedback and advances. After the last round the bot sends `🎯 You scored X/N`. Game state is in-memory only (a restart abandons in-flight games), and only one game per chat at a time — a second `/games` while a game is in progress replies `you have a game in progress`. With fewer than 4 vocab rows the menu is suppressed and the bot replies `add at least 4 words to your vocab first`; with a label spec that yields fewer than 4 playable rows the bot replies ``no words match those labels — try fewer filters or `/label` more words`` instead. Malformed spec → `⚠️ malformed label spec: …` and no picker. |
| `/irregulars` | Practise irregular verbs against a built-in static set (same for every chat). Runs `min(10, len(table))` rounds; each round shows the bare base form (e.g. `go`) and asks the user to type the past simple and past participle as plain text (`went / gone`, also accepted comma- or whitespace-separated, case-insensitive). Wrong answers reply with the canonical correct trio. After the last round the bot sends `🎯 You scored X/N`. Same one-game-per-chat semantics as `/games` (in-flight state is in-memory only); a second `/irregulars` while a game is in progress replies `you have an irregular-verbs game in progress`. |
...
- **`irregular_verbs.py`** — pure scaffolding for `/irregulars`: a static `IRREGULAR_VERBS` table of `(base, past_simple_alts, past_participle_alts)` (same for every chat), `Round`/`Game` dataclasses, `draw_rounds(...)` (sample `min(10, N)` verbs without replacement), `grade_answer(text, rd)` (split on `/`, `,`, whitespace; case-insensitive match against the alt lists), `apply_answer`, `format_result`. No telegram imports; `bot.py` wires `/irregulars` and intercepts plain-text replies in `handle_message` while a game is in flight.
- **`label_suggestor.py`** — LLM-driven label suggestions for the `/add` follow-up: `suggest_labels(word, existing_labels, llm_chat)` returns an ordered list `[pos:<value>, <existing label>, …]` capped at 5; `parse_llm_response(raw, existing_labels)` is the pure JSON parser; `PendingLabel` is the token↔(chat_id, word_id, label_name) registry behind the inline buttons. Any LLM/parse failure yields `[]` so the user-facing add reply is never affected.
