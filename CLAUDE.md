...
| `/translate <text>` | Google-translate the args (or, if sent as a reply, the replied message) into the chat's configured target language. If the input is written in the target's script (e.g. Cyrillic for `ru`), reverse-translate it to English instead and add the English result to this chat's vocab — unless the original phrase is longer than 5 words, in which case the note `not added (N words)` is shown. Duplicates surface as `already in vocab`. Does **not** invoke the LLM. Reverse detection only fires for non-Latin targets. |
