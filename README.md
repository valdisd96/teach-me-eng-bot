...
**System-managed labels.** Two label names are reserved for the bot and **cannot be attached or detached by hand** — `/label foo remembered` and `/unlabel foo focus:hard` both reject with `⚠️ … is a system label — managed automatically, can't be set by hand`.
